...
// import { AirVent, Bath, Bed, BedDouble, Castle, Ghost, Loader2, MountainSnow, Pencil, Plus, Ship, Trash, Trees, Tv, Users, UtensilsCrossed, VolumeX, Wifi } from "lucide-react";
...
//                 <AmenityItem><Users className="h-4 w-4"/>{room.guestCount} Bed{"(s)"}</AmenityItem>
//                 <AmenityItem><Bath className="h-4 w-4"/>{room.bathroomCount} Bed{"(s)"}</AmenityItem>
//                 {!!room.kingBad && <AmenityItem><BedDouble className="h-4 w-4"/>{room.kingBed} king Bed{"(s)"}</AmenityItem>}
...
//                 {room.ciryView && <AmenityItem><Castle className="h-4 w-4"/>City View</AmenityItem>}
...
//                 <Button disabled={isLoading} type="button" variant="ghost" ocClick={() => handleRoomDelete(room)}>
